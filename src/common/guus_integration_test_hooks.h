...
, "guus-default-integration-test-mem-name"
, false
};
...
void guus::use_standard_cout()   { if (!global_std_cout) { global_std_cout = std::cout.rdbuf(); } std::cout.rdbuf(global_std_cout); }
void guus::use_redirected_cout() { if (!global_std_cout) { global_std_cout = std::cout.rdbuf(); } std::cout.rdbuf(global_redirected_cout.rdbuf()); }

void guus::init_integration_test_context(const std::string &base_name)
{
  ...
  while (global_stdin_shared_mem->Open() != 0)
  {
    ...
    {
      once_only = false;
      printf("Guus Integration Test: Shared memory %s has not been created yet, blocking ...\n", global_stdin_shared_mem->Path().c_str());
    }
  }
  ...
  if (!global_stdin_semaphore_handle)  fprintf(stderr, "Guus Integration Test: Failed to initialise global_stdin_semaphore_handle\n");
  if (!global_stdout_semaphore_handle) fprintf(stderr, "Guus Integration Test: Failed to initialise global_stdout_semaphore_handle\n");
  if (!global_stdout_ready_semaphore) fprintf(stderr, "Guus Integration Test: Failed to initialise global_stdout_ready_semaphore_handle\n");
  if (!global_stdin_ready_semaphore) fprintf(stderr, "Guus Integration Test: Failed to initialise global_stdin_ready_semaphore_handle\n");

  printf("Guus Integration Test: Hooks initialised into shared memory, %s, %s, %s, %s, %s, %s\n",
      stdin_name.c_str(),
      stdout_name.c_str(),
...
void guus::deinit_integration_test_context()
{
...
std::vector<std::string> guus::separate_stdin_to_space_delim_args(guus::fixed_buffer const *cmd)
{
...
guus::fixed_buffer guus::read_from_stdin_shared_mem()
{
...
void guus::write_redirected_stdout_to_shared_mem()
{
  boost::unique_lock<boost::mutex> scoped_lock(integration_test_mutex);
