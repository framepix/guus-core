...
curl --header 'PRIVATE-TOKEN: '"$TOKEN" "https://gitlab.com/api/v4/projects/7515512/jobs" |
  ...
  do
    ...
    curl -L --header 'PRIVATE-TOKEN: '"$TOKEN" "https://gitlab.com/guusproject/guus/-/jobs/$id/artifacts/download" -o artifacts-$build.zip
    unzip artifacts-$build.zip
    mv build/release/bin guus-$build-x64-$version
    zip -r guus-$build-x64-$version.zip guus-$build-x64-$version
  done

echo '#### sha256sum'
sha256sum guus-*-x64-$version.zip
